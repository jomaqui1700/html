...
export async function GET(){const a=await auth();if(a.error)return a.error;try{const products=await sql`SELECT * FROM public.coffee_inventory_summary ORDER BY active DESC,category,product_name`;const movements=await sql`SELECT m.*,i.name AS product_name,i.category,i.unit,f.name AS farm_name,u.full_name AS created_by_name FROM public.coffee_inventory_movements m JOIN public.coffee_inventory i ON i.id=m.inventory_id LEFT JOIN public.farms f ON f.id=m.farm_id LEFT JOIN public.users u ON u.id=m.created_by ORDER BY m.movement_date DESC,m.id DESC LIMIT 500`;return NextResponse.json({products,movements})}catch(e){return dbError(e)}}
...
export async function POST(req){const a=await auth();if(a.error)return a.error;if(a.session.role!=='admin')return NextResponse.json({error:'Solo administrador.'},{status:403});try{const b=await req.json();
 if(b.action==='entrada'){
  ...
  const r=await sql`INSERT INTO public.coffee_inventory_movements(inventory_id,movement_date,movement_type,quantity,unit_cost,farm_id,supplier,notes,created_by) SELECT i.id,${movement_date},'entrada',${quantity},${unit_cost},NULL,${supplier||null},${notes||null},${a.session.id} FROM public.coffee_inventory i WHERE i.id=${id} AND i.active=true RETURNING id`;
